...
async function syncWithMongo(uid: string, email: string, nombre: string, foto: string) {
    const res = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ uid, email, nombre, foto }),
    });
    return res.json(); // { usuario, nuevo }
}
...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const u = cred.user;
            const { usuario, nuevo } = await syncWithMongo(
                u.uid,
                u.email ?? email,
            ...
            // Si es usuario nuevo o no completó onboarding, mandarlo al onboarding
            if (nuevo || !usuario.zonas?.length) {
    ...
    const handleForgotPassword = async () => {
        ...
        try {
            ...
            setError("No pudimos enviar el email. Verificá que el email sea correcto.");
        }
    };
            ...
            ">
                <div className="w-full text-center">
                    ...
                    <div className="flex justify-center -mb-6 lg:-mb-10">
                        <Image src="/logo.png" alt="FiguMatch" width={600} height={200}
                            className="w-[320px] lg:w-[550px] h-auto object-contain" priority />
                    </div>
                    ...
                    {/* Error global */}
                    {error && (
                        <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 rounded-2xl text-sm text-red-600 font-semibold text-left">
                            ⚠️ {error}
                        </div>
                    )}
                    ...
                    {/* Reset enviado */}
                    {resetSent && (
                        <div className="mb-4 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-2xl text-sm text-emerald-700 font-semibold text-left">
                    ...
                    <button onClick={handleGoogle} disabled={loading}
                        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-2xl border-2 border-gray-200 bg-white hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-[0.98] shadow-sm mb-5 group disabled:opacity-60">
                        <GoogleIcon />
                    ...
                    <form onSubmit={handleLogin} className="flex flex-col gap-3">
                        ...
                        <div className="flex justify-end -mt-1">
                            <button type="button" onClick={handleForgotPassword}
                                className="text-xs text-sky-500 font-semibold hover:text-sky-600 transition-colors">
