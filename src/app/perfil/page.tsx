...
import { getOnboardingData, completeOnboarding, resetOnboarding } from "@/lib/onboardingStore";
...
// ─── Zone selector modal ──────────────────────────────────────────────────────
function ZonaModal({ zonasActuales, onSave, onClose }: {
    ...
    const toggle = (zona: string) => {
        setSeleccionadas(prev => {
            ...
            if (prev.length >= MAX_ZONAS) return prev;
            return [...prev, zona];
        });
    };
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
            ...
            <div className="relative w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
                <div className="bg-gradient-to-r from-sky-500 to-sky-600 px-5 pt-5 pb-4 flex-shrink-0">
                    ...
                    <p className="text-sky-100 text-xs mt-0.5">Elegí hasta {MAX_ZONAS} zonas</p>
                </div>
                ...
                <div className="px-4 py-2 flex-shrink-0">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Buscar zona..." value={busqueda}
                ...
                <div className="overflow-y-auto flex-1 px-4 pb-2">
                    <div className="grid grid-cols-2 gap-1.5">
                        {filtradas.map(zona => {
                            ...
                            return (
                                <button key={zona} onClick={() => toggle(zona)} disabled={disabled}
                                    className={`text-left px-3 py-2 rounded-xl text-sm font-semibold transition-all border-2
                                        ${sel ? "bg-sky-50 border-sky-400 text-sky-700"
                ...
                <div className="px-4 py-4 border-t border-gray-100 flex gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => { onSave(seleccionadas); onClose(); }}
                        className="flex-1 py-3 rounded-xl bg-gradient-to-r from-sky-500 to-sky-600 text-white font-black text-sm active:scale-[0.98]">
                        Guardar zonas
                    </button>
                </div>
            </div>
...
// ─── Edit name modal ──────────────────────────────────────────────────────────
function EditNameModal({ nombreActual, onSave, onClose }: {
    nombreActual: string;
            ...
            <div className="relative w-full sm:max-w-sm bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="px-5 py-5">
                    ...
                    <div className="flex gap-2">
                        <button onClick={onClose} className="flex-1 py-3 rounded-xl border-2 border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-50">Cancelar</button>
                        <button onClick={() => nombre.trim() && onSave(nombre.trim())} disabled={!nombre.trim()}
                            className="flex-1 py-3 rounded-xl bg-gradient-to-r from-sky-500 to-sky-600 text-white font-black text-sm disabled:opacity-50 active:scale-[0.98]">
                            Guardar
                        </button>
                    </div>
                </div>
...
export default function PerfilPage() {
    ...
    const [zonas, setZonas] = useState<string[]>(session.zonas ?? []);
    const [foto, setFoto] = useState<string>(session.foto ?? "");
    const [reputacion, setReputacion] = useState(0);
    const [cambiosHechos, setCambiosHechos] = useState(0);
    ...
    useEffect(() => {
        if (!mongoId) { setLoadingData(false); return; }
        fetch(`/api/usuarios/${mongoId}`)
            ...
            .then(u => {
                ...
                setZonas(u.zonas ?? []);
                setFoto(u.foto ?? "");
                setReputacion(u.reputacion ?? 0);
                setCambiosHechos(u.cambiosHechos ?? 0);
                setCiudad(u.ciudad ?? "");
            })
            .finally(() => setLoadingData(false));
    }, [mongoId]);

    // ── Guardar cambios en MongoDB y localStorage ─────────────────────────────
    const guardar = async (patch: Record<string, unknown>) => {
        ...
        try {
            ...
            completeOnboarding({
                ...
                foto: u.foto,
                zonas: u.zonas,
                repetidas: u.repetidas,
                faltantes: u.faltantes,
            });
            setSavedToast(true);
    ...
    const handleSaveNombre = async (n: string) => {
        setNombre(n);
        setShowNameModal(false);
        await guardar({ nombre: n });
    };

    const handleSaveZonas = async (z: string[]) => {
        setZonas(z);
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-gray-400 font-medium">Cargando perfil...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="px-4 pt-20 pb-24 lg:ml-64 lg:pt-8 lg:px-8 lg:pb-10 xl:mr-72">
                <div className="max-w-lg mx-auto lg:max-w-xl lg:mx-auto">
                            ...
                            <div className="relative flex-shrink-0">
                                <div className="w-24 h-24 rounded-2xl overflow-hidden shadow-lg ring-4 ring-white/30">
                                    {foto ? (
                                        ...
                                        <img src={foto} alt="Foto de perfil" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                    ) : (
                                ...
                                <button onClick={() => fileInputRef.current?.click()} disabled={saving}
                                    className="absolute -bottom-2 -right-2 w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center hover:bg-gray-50 transition-colors border border-gray-100 disabled:opacity-60"
                                    title="Cambiar foto">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {ciudad && (
                                    <div className="flex items-center gap-1 mb-3">
                                        ...
                                        <span className="text-sky-100 text-sm">{ciudad}</span>
                                    </div>
                                )}
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="bg-white/15 rounded-xl px-3 py-1.5 text-center">
                                        <div className="text-amber-300 font-black text-lg leading-none">{reputacion > 0 ? reputacion.toFixed(1) : "—"}</div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                        ...
                        {reputacion > 0 ? (
                            ...
                            <p className="text-sm text-gray-400 font-medium">Completá tu primer intercambio para obtener reputación. 🤝</p>
                        )}
                        <div className="mt-4 flex items-start gap-2 bg-amber-50 border border-amber-100 rounded-xl p-3">
                            ...
                            <p className="text-xs text-amber-700 font-medium">Tu reputación sube cuando completás intercambios. ¡Seguí canjeando!</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => setShowZonaModal(true)}
                        ...
                        {zonas.length === 0 ? (
                            ...
                            <>
                                ...
                                <div className="mt-3 flex items-start gap-2 bg-sky-50 border border-sky-100 rounded-xl p-3">
                                    ...
                                    <p className="text-xs text-sky-700 font-medium">Solo aparecerás en el feed de usuarios que compartan al menos una zona con vos.</p>
                                </div>
                            </>
                        )}
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            ...
                            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 text-center">
                                <div className="text-2xl font-black text-amber-600 leading-none">{reputacion > 0 ? reputacion.toFixed(1) : "—"}</div>
                    ...
                    <button
                        ...
                        disabled={loggingOut}
                        className="w-full mt-4 flex items-center justify-center gap-2 py-3.5 rounded-2xl border-2 border-red-100 bg-red-50 hover:bg-red-100 hover:border-red-200 text-red-500 font-black text-sm transition-all active:scale-[0.98] disabled:opacity-60"
                    >
                        {loggingOut ? (
            ...
            {showZonaModal && <ZonaModal zonasActuales={zonas} onSave={handleSaveZonas} onClose={() => setShowZonaModal(false)} />}
            {showNameModal && <EditNameModal nombreActual={nombre} onSave={handleSaveNombre} onClose={() => setShowNameModal(false)} />}
